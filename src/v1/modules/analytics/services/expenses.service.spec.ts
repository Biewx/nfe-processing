...
describe("ExpensesService", () => {
    ...
    it("busca o total de gastos sem nenhum filtro quando nenhum param é informado", async () => {
        // Arrange
        ...
        // deve ser um objeto vazio — nenhum filtro aplicado
    ...
    it("filtra por supplierId quando ele é informado", async () => {
        ...
        // Act
        await service.getTotalExpenses({ supplierId: 7 });

        // Assert
        expect(fakeRepository.getTotalExpenses).toHaveBeenCalledWith({ supplierId: 7 });
    });
    ...
    it("filtra por período quando month e year são informados", async () => {
        ...
        // Act: agosto de 2026
        await service.getTotalExpenses({ month: 8, year: 2026 });
    ...
    it("busca a maior despesa aplicando o mesmo filtro de where", async () => {
        ...
        // Act
        const result = await service.getHighestExpenses({ supplierId: 3 });

        // Assert
        expect(fakeRepository.getHighestExpenses).toHaveBeenCalledWith({ supplierId: 3 });
